...
# Network addresses come from the canonical deployments.json published by
# Cyfrin/battlechain-lib, fetched at startup with an embedded offline fallback.

CHAIN_ID = "627"  # BattleChain testnet — the network this demo targets

_DEPLOYMENTS_URL = (
    "https://raw.githubusercontent.com/Cyfrin/battlechain-lib/main/deployments.json"
)

# Snapshot of the deployments.json fields used below, for when the fetch fails.
# Update if the testnet is redeployed; the live fetch supersedes this when online.
_DEPLOYMENTS_FALLBACK = {
    "626": {
...
def _load_deployments() -> dict:
    """Return deployments.json["networks"], or the offline fallback on failure."""
    try:
        with urllib.request.urlopen(_DEPLOYMENTS_URL, timeout=10) as resp:
            return json.loads(resp.read().decode())["networks"]
    except Exception as exc:  # network error, bad JSON, or missing key
        print(f"[battlechain] deployments.json fetch failed ({exc}); "
              "using embedded fallback", file=sys.stderr)
        return _DEPLOYMENTS_FALLBACK
...
def _resolve_deployment(networks: dict, chain_id: str) -> dict:
    ...
    """
    net = networks.get(chain_id) or _DEPLOYMENTS_FALLBACK.get(chain_id, {})
